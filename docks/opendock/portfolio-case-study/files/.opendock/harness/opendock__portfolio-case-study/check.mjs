...
const config = {
  ...
  runDir: ".opendock/runs/portfolio-case-study",
  outputPrefix: "portfolio/",
  extensions: new Set([".md", ".mdx", ".txt", ".json", ".yml", ".yaml", ".html"]),
  reportSections: [
    ["Background", "배경"],
    ["Problem", "문제"],
    ["Role / Constraints", "역할 / 제약", "역할과 제약"],
    ["Research / Evidence", "리서치 / 근거", "리서치와 근거"],
    ["Decisions / Process", "의사결정 / 과정", "의사결정과 과정"],
    ["Solution", "해결안", "솔루션"],
    ["Results", "결과"],
    ["Reflection", "회고"],
    ["Privacy / Redaction", "개인정보 / 비식별화", "개인정보와 비식별화"]
  ],
  manifestSections: [
    ["Background", "배경"],
    ["Problem", "문제"],
...
function checkPortfolioEvidence(file, text) {
  const results = getSection(text, ["Results", "결과"]) ?? "";
  const hasEvidence = /(?:\bEvidence\s*[:：]|근거\s*[:：]|증거\s*[:：]|측정 근거)/i.test(results);
  const hasProxy = /(?:\bProxy\s*[:：]|대리\s*지표\s*[:：]|프록시\s*[:：])/i.test(results);
  const hasLimitation = /(?:\bLimitations?\b|한계|제약)/i.test(results);
  if (!hasEvidence && !(hasProxy && hasLimitation)) {
    fail("unsupported-results", file, "Results에는 evidence 또는 한계를 밝힌 honest proxy가 필요합니다.");
  }

  const metricPattern = /(?:\d+(?:\.\d+)?\s*(?:%|퍼센트|명|users?|hours?|days?|시간|일|원|달러)|[$₩]\s*\d)/i;
  if (metricPattern.test(results) && !/(?:\bSource\s*[:：]|출처\s*[:：]|측정\s*기간|analytics\s+export|experiment|survey|실험|설문)/i.test(results)) {
    fail("fabricated-metric-risk", file, "수치 결과에는 source 또는 측정 기간이 필요합니다.");
  }

  const privacy = getSection(text, ["Privacy / Redaction", "개인정보 / 비식별화", "개인정보와 비식별화"]) ?? "";
  if (!/(?:redact|비식별|제거|범주|합성|PII|개인정보|공개 범위)/i.test(privacy)) {
    fail("missing-redaction-evidence", file, "Privacy / Redaction에는 실제 최소화 또는 비식별화 조치가 필요합니다.");
  }
}
...
function validateManifest(manifestRel) {
  ...
  checkPortfolioEvidence(manifestRel, text);
  ...
  if (report !== undefined) {
    ...
    checkPortfolioEvidence(primary, report);
  }
}
