...
const slug = "trip-planner";
const title = "Trip Planner";
const outputRoot = "trips";
...
const requiredSections = [
  "요청 및 범위 (Request and Scope)",
  "여행자·일정·예산 (Traveler, Dates, Budget)",
  "선호·제약 (Preferences and Constraints)",
  sourceHeading,
  factHeading,
  "경로 현실성 근거 (Route Realism Evidence)",
  "예약·가격 검증 (Reservation and Price Checks)",
  "날씨·휴무·우천 대안 (Weather, Closures, Rain Fallback)",
  "안전·비상·개인정보 (Safety, Emergency, Privacy)",
  "미결정 사항 (Unresolved Decisions)",
  targetHeading,
  "검증 결과 (Validation)",
];
const outputHeadings = [
  ["여행자", /여행자|travelers?/i],
  ["날짜 또는 기간", /날짜|기간|dates?|duration/i],
  ["예산", /예산|budget/i],
  ["선호", /선호|preferences?/i],
  ["제약", /제약|constraints?/i],
  ["일정표", /일정표|여정|itinerary/i],
  ["동선 현실성", /동선|이동 현실성|route realism/i],
  ["예산 내역", /예산 내역|비용 내역|budget breakdown|cost breakdown/i],
  ["예약", /예약|reservations?/i],
  ["준비물", /준비물|짐 목록|packing(?: list)?/i],
  ["날씨와 휴무", /날씨.*휴무|휴무.*날씨|기상.*휴무|weather.*closures?|closures?.*weather/i],
  ["우천 대안", /우천|rain fallback|rainy-day alternative/i],
  ["안전과 비상", /안전.*비상|비상.*안전|safety.*emergency|emergency.*safety/i],
  ["미결정 사항", /미결정|결정 필요|unresolved decisions?|open decisions?/i],
  ["출처", /출처|sources?/i],
...
function validateSources(run) {
  ...
  if (sourceLines.length < 2) {
    fail("source-count", file, "교통·날씨·운영 검증을 위한 HTTPS 출처가 두 개 이상 필요합니다.");
  }
...
function validateOutput(targets, run) {
  ...
  const times = cleaned.match(/\b(?:[01]?\d|2[0-3]):[0-5]\d\b/g) ?? [];
  if (times.length < 2 || !/\b\d+(?:\.\d+)?\s*(?:분|시간|minutes?|hours?)/i.test(cleaned)) {
    fail("route-realism", targets[0].relative, "일정에는 시각 두 개 이상과 구간 이동 시간이 필요합니다.");
  }
  const amounts = cleaned.match(/(?:[₩$€¥]\s*\d[\d,.]*|\d[\d,.]*\s*(?:원|엔|달러|유로|USD|JPY|KRW|EUR))/gi) ?? [];
  if (amounts.length < 2) fail("budget-breakdown", targets[0].relative, "예산 내역에는 통화가 명시된 금액이 두 개 이상 필요합니다.");
  if (!/(?:가격|요금|예약|영업시간|가용성|price|fare|reservation|availability|opening hours)[^.\n]{0,80}(?:변동|재확인|직접 확인|추정|예상|change|vary|recheck|verify|estimate)/i.test(cleaned)) {
    fail("reservation-caveat", targets[0].relative, "가격·예약·운영 정보에 변동 가능성과 재확인 안내가 필요합니다.");
  }
  if (!/[-*]\s+\[[ xX]\]/.test(cleaned)) fail("packing-checklist", targets[0].relative, "준비물은 체크 가능한 목록이어야 합니다.");
  ...
  if (/(?:가격|요금|예약\s*가능(?:성)?|좌석|객실)[^.\n]{0,35}(?:보장|확정되어\s*있|절대\s*변하지|항상\s*가능)|(?:guaranteed|confirmed)\s+(?:price|availability)/i.test(cleaned)) {
    fail("unsupported-guarantee", targets[0].relative, "가격 또는 예약 가능성을 근거 없이 보장할 수 없습니다.");
  }
