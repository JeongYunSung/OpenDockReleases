...
const slug = "travel-research";
const title = "Travel Research";
const outputRoot = "travel-research";
...
const sourceHeading = "시간 민감 출처 (Time-sensitive Sources)";
const factHeading = "사실·가정·추천 구분 (Facts, Assumptions, Recommendations)";
const requiredSections = [
  "조사 질문과 범위 (Research Question and Scope)",
  "목적지·체류·날짜 (Destination, Stay Length, Dates)",
  sourceHeading,
  factHeading,
  "지역·교통 근거 (Neighborhood and Transport Evidence)",
  "계절·안전·현지 규칙 근거 (Season, Safety, Local Rules Evidence)",
  "결제·연결·비용 근거 (Payments, Connectivity, Cost Evidence)",
  "관광객 함정·불확실성·상충관계 (Tourist Traps, Uncertainty, Tradeoffs)",
  "개인정보 최소화·가림 (Data Minimization and Redaction)",
  targetHeading,
  "검증 결과 (Validation)",
];
const outputHeadings = [
  ["목적지", /목적지|destination/i],
  ["체류 길이와 날짜", /체류.*(?:날짜|기간)|(?:날짜|기간).*체류|stay length.*dates?|dates?.*stay length/i],
  ["동네 비교", /동네|지역 비교|neighborhoods?|area comparison/i],
  ["교통", /교통|transport/i],
  ["날씨와 계절", /날씨.*계절|계절.*날씨|기상.*계절|weather.*season|season.*weather/i],
  ["안전", /안전|safety/i],
  ["현지 규칙과 예절", /(?:현지 )?규칙.*예절|예절.*(?:현지 )?규칙|local rules?.*etiquette|etiquette.*local rules?/i],
  ["결제와 연결", /결제.*(?:연결|통신)|(?:연결|통신).*결제|payments?.*connectivity|connectivity.*payments?/i],
  ["비용", /비용|가격대|costs?|price range/i],
  ["관광객 함정", /관광객 함정|tourist traps?/i],
  ["출처와 접근일", /출처.*접근일|접근일.*출처|sources?.*access dates?|access dates?.*sources?/i],
  ["불확실성과 상충관계", /불확실성.*상충관계|상충관계.*불확실성|추천 tradeoff|uncertainty.*tradeoffs?|tradeoffs?.*uncertainty/i],
...
function validateSources(run) {
  ...
  if (sourceLines.length < 3) {
    fail("source-count", file, "교통·기상·현지 규정 검증을 위한 HTTPS 출처가 세 개 이상 필요합니다.");
  }
...
function validateOutput(targets, run) {
  ...
  const amounts = cleaned.match(/(?:[₩$€¥]\s*\d[\d,.]*|\d[\d,.]*\s*(?:원|엔|달러|유로|USD|JPY|KRW|EUR))/gi) ?? [];
  if (amounts.length < 2) fail("cost-evidence", targets[0].relative, "비용 비교에는 통화가 명시된 금액 또는 범위가 두 개 이상 필요합니다.");
  ...
  if (!/(?:장점|이점|benefit|advantage|upside)/i.test(cleaned) || !/(?:단점|위험|비용|drawback|risk|downside|cost)/i.test(cleaned)) {
    fail("recommendation-tradeoff", targets[0].relative, "추천에는 장점과 단점·비용·위험을 함께 적어야 합니다.");
  }
  if (!/(?:관광객 함정|tourist traps?)[\s\S]{0,500}(?:대안|확인|회피|alternative|verify|avoid)/i.test(cleaned)) {
    fail("tourist-trap-evidence", targets[0].relative, "관광객 함정에는 검증 방법 또는 대안이 필요합니다.");
  }
  ...
  if (outputSources.length < 2 || outputSources.some((line) => !/(?:접근일|accessed)\s*:?\s*\d{4}-\d{2}-\d{2}/i.test(line))) {
    fail("output-sources", targets[0].relative, "산출물에도 접근일이 대응된 HTTPS 출처가 두 개 이상 필요합니다.");
  }
  ...
  if (/(?:절대\s*안전|무조건\s*최고|가격[^.\n]{0,25}(?:보장|확정)|(?:항상|확실히)\s*(?:영업|운행)|guaranteed\s+(?:safe|price|open))/i.test(cleaned)) {
    fail("unsupported-guarantee", targets[0].relative, "안전·가격·영업·추천을 절대적으로 보장할 수 없습니다.");
  }
