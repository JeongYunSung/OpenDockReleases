...
1. `.opendock/docks/security-privacy-ultrawork/SECURITY_PRIVACY.md`를 읽습니다.
2. 이번 작업의 작업 식별자를 정합니다.
3. 템플릿을 복사합니다.

```bash
mkdir -p .opendock/runs/security-privacy
cp .opendock/templates/security-privacy/SECURITY_PRIVACY_RUN.md .opendock/runs/security-privacy/<작업-id>.md
```
