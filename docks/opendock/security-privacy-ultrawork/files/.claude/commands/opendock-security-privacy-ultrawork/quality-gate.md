...
2. current run 문서에 `node .opendock/harness/security-privacy-ultrawork/check.mjs --target <run-document>`를 실행하고 승인된 예외만 기록합니다. 프로젝트 전체 검수를 명시한 경우에만 `--release`를 사용합니다.
