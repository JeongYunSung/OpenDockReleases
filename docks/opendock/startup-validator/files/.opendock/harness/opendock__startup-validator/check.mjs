...
const config = {
  ...
  runRoot: ".opendock/runs/startup-validator",
...
function validateSafety(text, file) {
  ...
  for (const line of actionable.split(/\r?\n/)) {
    if (!/(?:market\s+size|\bTAM\b|\bSAM\b|\bSOM\b|시장\s*규모).{0,100}(?:\$\s*)?\d[\d,.]*\s*(?:%|[KMB]\b|원|달러|명|억|조)?/i.test(line)) continue;
    if (/\[S\d+\]|https?:\/\/|source|evidence|근거|출처/i.test(line)) continue;
    addFailure("unsupported-market-size", file, "A numeric market-size claim needs a same-line source marker or URL.");
    break;
  }
}
...
function validateRunManifest(run) {
  ...
  requireTopField(text, file, "Venture");
  ...
  requireSectionField(text, file, "Threshold Evidence", "Threshold Rationale");
  requireSectionField(text, file, "Decision Evidence", "Decision Rule");
  ...
  if (validation && !/(?:pass|fail|insufficient|passed|failed|통과|실패|불충분|확인|완료)/i.test(validation)) {
    addFailure("invalid-validation-result", file, "Validation Result must record pass, fail, or insufficient evidence.");
  }
...
function validateStartupOutput(targets, runFile) {
  ...
  const file = runFile;
  requireOutputTopField(combined, file, "Venture");
  ...
  for (const heading of [
    ...
    if (!hasEvidence(value)) addFailure("missing-output-section", file, `${heading} needs concrete validation content.`);
  }

  requireOutputSectionField(combined, file, "Problem Hypothesis", "Problem Statement");
  requireOutputSectionField(combined, file, "Problem Hypothesis", "Observed Pain");
  requireOutputSectionField(combined, file, "ICP", "Primary ICP");
  requireOutputSectionField(combined, file, "ICP", "Trigger Context");
  requireOutputSectionField(combined, file, "Current Alternatives", "Alternative");
  requireOutputSectionField(combined, file, "Current Alternatives", "Switching Friction");
  requireOutputSectionField(combined, file, "Risky Assumptions", "Risky Assumption");
  requireOutputSectionField(combined, file, "Risky Assumptions", "Why Risky");
  ...
  for (const name of ["Validation Method", "Sample", "Timebox"]) {
    requireOutputSectionField(combined, file, "Validation Method", name);
  }
  const questionLines = sectionText(combined, "Interview Questions")
    .split(/\r?\n/)
    .filter((line) => /^\s*(?:[-*]|\d+[.)])\s+\S/.test(line));
  if (questionLines.length < 3) addFailure("insufficient-interview-questions", file, "Interview Questions must contain at least three concrete questions.");
  requireOutputSectionField(combined, file, "Pass / Fail Thresholds", "Pass Threshold");
  requireOutputSectionField(combined, file, "Pass / Fail Thresholds", "Fail Threshold");
  requireOutputSectionField(combined, file, "MVP Scope", "In Scope");
  requireOutputSectionField(combined, file, "MVP Non-goals", "Non-goal");
  requireOutputSectionField(combined, file, "Pricing Hypothesis", "Pricing Hypothesis");
  requireOutputSectionField(combined, file, "Pricing Hypothesis", "Pricing Test");
  requireOutputSectionField(combined, file, "Next Decision", "Decision Trigger");
  requireOutputSectionField(combined, file, "Next Decision", "Next Decision");
  requireOutputSectionField(combined, file, "Safety / Limitations", "Limitations");
}
...
function main() {
  ...
  validateStartupOutput(targets, run.relativePath);
