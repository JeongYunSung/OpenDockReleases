...
const requiredManagedDocs = [
  ".opendock/docks/creative-gen-ultrawork/HARNESS.md",
...
function assertSafeExistingPath(rel) {
  if (!rel || path.isAbsolute(rel)) throw new Error(`안전하지 않은 경로입니다: ${rel}`);
  const normalized = path.normalize(rel);
  if (normalized === ".." || normalized.startsWith(`..${path.sep}`)) {
    throw new Error(`프로젝트 밖 경로는 사용할 수 없습니다: ${rel}`);
  }
  ...
  for (const segment of normalized.split(path.sep)) {
    ...
    if (!fs.existsSync(current)) throw new Error(`경로가 없습니다: ${rel}`);
    if (fs.lstatSync(current).isSymbolicLink()) {
      throw new Error(`심볼릭 링크 경로는 사용할 수 없습니다: ${normalize(current)}`);
    }
  }
...
function walk(dir, depth = 0, state = { entries: 0, stopped: false }) {
  ...
  try {
    assertSafeExistingPath(normalize(dir));
  } catch (error) {
    recordTraversalFailure("unsafe-walk-path", normalize(dir), error instanceof Error ? error.message : String(error));
    return out;
  }
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink()) {
      recordTraversalFailure("walk-symlink", normalize(full), "Directory traversal does not follow symbolic links.");
      continue;
    }
...
function filesFromManifestPaths(outputPaths, dirs, exts) {
  const allowedPrefixes = dirs.map((dir) => `${dir.replace(/\/$/, "")}/`);
  const files = [];
  for (const rel of outputPaths) {
    ...
    try {
      assertSafeExistingPath(rel);
    } catch (error) {
      recordTraversalFailure("unsafe-output-path", rel, error instanceof Error ? error.message : String(error));
      continue;
    }
    const stat = fs.lstatSync(full);
    if (stat.isFile()) files.push({ full, rel, ext });
  }
  return files;
}
...
function findRunDocuments() {
  ...
  if (exists("GENERATION_BRIEF.md") && exists("OUTPUT_MANIFEST.md")) {
    assertSafeExistingPath("GENERATION_BRIEF.md");
    assertSafeExistingPath("OUTPUT_MANIFEST.md");
    docs.push({
  ...
  assertSafeExistingPath(runRoot);

  for (const entry of fs.readdirSync(fullRunRoot, { withFileTypes: true })) {
    if (entry.isSymbolicLink()) {
      recordTraversalFailure("run-symlink", `${runRoot}/${entry.name}`, "작업 폴더는 심볼릭 링크일 수 없습니다.");
      continue;
    }
    ...
    try {
      assertSafeExistingPath(briefRel);
      assertSafeExistingPath(manifestRel);
    } catch (error) {
      recordTraversalFailure("unsafe-run-document", runRel, error instanceof Error ? error.message : String(error));
      continue;
    }
    docs.push({
      label: runRel,
...
function loadRunDocument(manifestArg) {
  const manifestFull = path.resolve(root, manifestArg);
  const relative = normalize(manifestFull);
  const expectedPrefix = `${runRoot}/`;
  if (!relative.startsWith(expectedPrefix) || path.basename(relative) !== "manifest.md") {
    throw new Error(`작업 기록은 ${runRoot}/<작업-id>/manifest.md 형식이어야 합니다.`);
  }

  const briefRel = `${path.posix.dirname(relative)}/brief.md`;
  for (const rel of [relative, briefRel]) {
    assertSafeExistingPath(rel);
  }

  return {
    label: path.posix.dirname(relative),
    briefRel,
    manifestRel: relative,
    briefText: readText(briefRel),
    manifestText: readText(relative),
    mtime: Math.max(fileMtime(briefRel), fileMtime(relative))
  };
}
...
function parseOptions(argv) {
  ...
  if (release && manifest) throw new Error("--release와 작업 기록 경로는 함께 사용할 수 없습니다.");
  if (argv.includes("--manifest") && !manifest) throw new Error("--manifest 뒤에 작업 기록 경로가 필요합니다.");
...
function validateMode(mode, manifestText, outputPaths, failures, run) {
  ...
  const exts = mode === "video" ? [...spec.exts, ".vtt", ".srt"] : spec.exts;
  const currentFiles = filesFromManifestPaths(outputPaths, spec.dirs, exts);
...
function run() {
  ...
  try {
    ...
    failures.push({ rule: "invalid-arguments", file: "command", detail: error instanceof Error ? error.message : String(error) });
  }

  if (!options || (!options.release && !options.manifest)) {
    if (failures.length > 0) {
      printFailures(failures, [], 0, "none");
      process.exit(1);
    }
    console.log(`OpenDock harness: ${title}`);
    console.log("Status: ready");
    console.log("검사할 작업 기록을 지정하면 해당 결과물만 확인합니다.");
    console.log("출시 전체 검사는 --release를 명시했을 때만 실행합니다.");
    return;
  }

  let runDocs = [];
  try {
    runDocs = options.release ? findRunDocuments() : [loadRunDocument(options.manifest)];
  } catch (error) {
    failures.push({ rule: "invalid-run-document", file: options.manifest || runRoot, detail: error instanceof Error ? error.message : String(error) });
  }

  const manifestPaths = new Set(runDocs.flatMap((run) => [...extractManifestPaths(run.manifestText)]));
  const allOutputFiles = options.release
    ? [
    ...
    : [
        ...filesFromManifestPaths(manifestPaths, modeSpecs.image.dirs, modeSpecs.image.exts),
        ...filesFromManifestPaths(manifestPaths, modeSpecs.vector.dirs, modeSpecs.vector.exts),
        ...filesFromManifestPaths(manifestPaths, modeSpecs.logo.dirs, modeSpecs.logo.exts),
        ...filesFromManifestPaths(manifestPaths, modeSpecs.favicon.dirs, modeSpecs.favicon.exts),
        ...filesFromManifestPaths(manifestPaths, modeSpecs.video.dirs, [...modeSpecs.video.exts, ".vtt", ".srt"]),
        ...filesFromManifestPaths(manifestPaths, modeSpecs.audio.dirs, modeSpecs.audio.exts)
      ];
