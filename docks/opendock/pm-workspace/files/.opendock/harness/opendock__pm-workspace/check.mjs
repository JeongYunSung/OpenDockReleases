...
const config = {
  title: "PM Workspace",
  slug: "pm-workspace",
  runRoot: ".opendock/runs/pm-workspace",
...
function validateRunManifest(run) {
  ...
  const date = requireTopField(text, file, "Decision Date");
  if (date && !isIsoDate(date)) addFailure("invalid-date", file, "Decision Date must be a valid YYYY-MM-DD date.");
  ...
  requireSectionField(text, file, "Source Separation Evidence", "Facts Basis");
  requireSectionField(text, file, "Source Separation Evidence", "Assumptions Basis");
  requireSectionField(text, file, "Source Separation Evidence", "Open Questions Basis");
  requireSectionField(text, file, "Acceptance Criteria Evidence", "Criteria Review");
  requireSectionField(text, file, "Metric Evidence", "Metric Review");
  requireSectionField(text, file, "Decision Evidence", "Decision Log Review");
...
function validatePmOutput(targets, runFile) {
  ...
  const date = requireOutputTopField(combined, file, "Decision Date");
  if (date && !isIsoDate(date)) addFailure("invalid-output-date", file, "Output Decision Date must be a valid YYYY-MM-DD date.");
  ...
  for (const heading of [
    ...
    const value = sectionText(combined, heading);
    if (!hasEvidence(value)) addFailure("missing-output-section", file, `${heading} needs concrete product content.`);
  }

  requireOutputSectionField(combined, file, "Facts", "Fact");
  requireOutputSectionField(combined, file, "Assumptions", "Assumption");
  requireOutputSectionField(combined, file, "Open Questions", "Open Question");
  requireOutputSectionField(combined, file, "Problem", "Problem Statement");
  requireOutputSectionField(combined, file, "Problem", "Problem Evidence");
  requireOutputSectionField(combined, file, "Users", "Primary User");
  requireOutputSectionField(combined, file, "Users", "User Need");
  requireOutputSectionField(combined, file, "Goals", "Goal");
  requireOutputSectionField(combined, file, "Non-goals", "Non-goal");
  for (const name of ["Metric", "Baseline", "Target", "Measurement"]) {
    requireOutputSectionField(combined, file, "Success Metrics", name);
  }
  requireOutputSectionField(combined, file, "Requirements", "Requirement ID", 3);
  requireOutputSectionField(combined, file, "Requirements", "Priority", 3);
  for (const name of ["User Story", "Acceptance Criteria", "Given", "When", "Then"]) {
    requireOutputSectionField(combined, file, "User Stories with Acceptance Criteria", name);
  }
  requireOutputSectionField(combined, file, "Edge Cases", "Edge Case");
  requireOutputSectionField(combined, file, "Edge Cases", "Expected Behavior");
  requireOutputSectionField(combined, file, "Risks", "Risk");
  requireOutputSectionField(combined, file, "Risks", "Mitigation");
  requireOutputSectionField(combined, file, "Dependencies", "Dependency");
  requireOutputSectionField(combined, file, "Dependencies", "Owner");
  const logDate = requireOutputSectionField(combined, file, "Release / Decision Log", "Date", 3);
  if (logDate && !isIsoDate(logDate)) addFailure("invalid-log-date", file, "Decision log Date must be a valid YYYY-MM-DD date.");
  requireOutputSectionField(combined, file, "Release / Decision Log", "Decision");
  requireOutputSectionField(combined, file, "Release / Decision Log", "Rationale");
  const validation = requireOutputSectionField(combined, file, "Validation Evidence", "Validation Result");
  if (validation && !/(?:pass|passed|통과|확인|완료)/i.test(validation)) {
    addFailure("invalid-output-validation", file, "Validation Result must record a completed review.");
  }
}
...
function main() {
  ...
  validatePmOutput(targets, run.relativePath);
