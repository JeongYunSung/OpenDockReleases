...
- Root documentation pollution: installing `README.md`, `HARNESS.md`, or `*PLAYBOOK*.md` at project root instead of the dock namespace.
- Dock type overreach: adding custom harness, HARNESS, or quality-gate workflow to Tool or regular workspace Docks instead of validating the actual tool or relying on concise domain guidance.
...
- Harnesses that scan the entire repo, score semantic quality with brittle heuristics, or may block unrelated work.
...
The checker parses YAML structurally with Bun, scans payload files recursively, rejects symlinks and prompt-injection patterns, normalizes Windows and POSIX paths, and redacts command contents from failure details.
