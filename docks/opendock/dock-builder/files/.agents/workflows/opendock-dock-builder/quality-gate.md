...
이 workflow는 사용자가 `검수` 또는 `ultrawork`를 명시한 경우 현재 Dock에만 실행합니다. 전체 컬렉션은 `전체 검수`를 명시한 경우에만 확인합니다.

1. `.opendock/docks/dock-builder/PLAYBOOK.md`와 대상 Dock의 목적을 확인합니다.
2. 정적 checker로 사용자 중심 카탈로그 설명, root 문서, namespaced README, Dock 유형, platform parity를 확인합니다.
...
9. 실패를 수정하거나 보류 사유를 남기기 전에는 검수가 끝났다고 말하지 않습니다.
