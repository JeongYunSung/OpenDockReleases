...
const config = {
  "title": "Data Ultrawork",
  "focus": "data and analytics quality gates",
  "patterns": [
    {
    ...
    },
    {
      ...
      "pattern": "(?i)select\\s+\\*",
      "message": "Avoid select * in shared data work."
    },
    {
      "id": "destructive-sql",
      "globs": [
      ...
      "pattern": "(?i)\\b(drop|truncate|delete\\s+from|alter\\s+table)\\b",
      "message": "Destructive SQL requires review."
    },
    {
      "id": "missing-timezone-copy",
      "globs": [
        "**/*.sql",
        "**/*.md"
      ],
      "pattern": "(?i)\\b(date|timestamp|created_at|updated_at)\\b(?![\\s\\S]{0,300}(timezone|UTC|KST|local time))",
      "message": "Date/time work should state timezone assumptions."
    },
    {
      "id": "pii-column",
      "globs": [
...
const ignoredRootFiles = new Set(["AGENTS.md", "CLAUDE.md", "GEMINI.md", ".opendock/docks/data-ultrawork/HARNESS.md", ".opendock/docks/data-ultrawork/README.md"]);
