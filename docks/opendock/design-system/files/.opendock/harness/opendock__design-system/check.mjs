...
const config = {
  title: "Design System",
  runDir: ".opendock/runs/design-system",
  outputPrefix: "design-system/",
  extensions: new Set([".md", ".mdx", ".txt", ".json", ".yml", ".yaml", ".css", ".scss", ".js", ".mjs", ".cjs", ".ts", ".tsx"]),
  reportSections: [
    ["Principles", "원칙"],
    ["Naming", "네이밍", "이름 규칙"],
    ["Color Tokens", "색상 토큰"],
    ["Type Tokens", "타입 토큰", "타이포그래피 토큰"],
    ["Spacing Tokens", "간격 토큰"],
    ["Radius Tokens", "반경 토큰", "모서리 토큰"],
  ...
  manifestSections: [
    ...
    ["Adoption Plan", "도입 계획"],
    ["Evidence", "근거"],
    ["Privacy / Sample Data", "개인정보 / 샘플 데이터", "개인정보와 샘플 데이터"]
  ]
};
...
function checkDesignSystemEvidence(file, text) {
  const tokenChecks = [
    {
      aliases: ["Color Tokens", "색상 토큰"],
      rule: "missing-color-roles",
      minimum: 5,
      terms: ["\\bcanvas\\b", "\\bsurface\\b", "\\btext\\b", "\\bborder\\b", "\\bprimary\\b", "\\bfocus\\b", "\\bsuccess\\b", "\\bwarning\\b", "\\berror\\b", "배경", "캔버스", "표면", "본문", "텍스트", "테두리", "주요", "초점", "포커스", "성공", "경고", "오류"]
    },
    {
      aliases: ["Type Tokens", "타입 토큰", "타이포그래피 토큰"],
      rule: "missing-type-roles",
      minimum: 3,
      terms: ["\\bdisplay\\b", "\\bheading\\b", "\\bbody\\b", "\\blabel\\b", "\\bcaption\\b", "\\bcode\\b", "제목", "본문", "레이블", "캡션", "코드"]
    },
    {
      aliases: ["Spacing Tokens", "간격 토큰"],
      rule: "missing-spacing-roles",
      minimum: 3,
      terms: ["\\binset\\b", "\\bstack\\b", "\\bgap\\b", "\\bsection\\b", "\\bcontrol\\b", "안쪽", "수직", "간격", "섹션", "컨트롤"]
    },
    {
      aliases: ["Radius Tokens", "반경 토큰", "모서리 토큰"],
      rule: "missing-radius-roles",
      minimum: 2,
      terms: ["\\bcontrol\\b", "\\bsurface\\b", "\\boverlay\\b", "컨트롤", "표면", "오버레이"]
    },
    {
      aliases: ["Shadow Tokens", "그림자 토큰"],
      rule: "missing-shadow-roles",
      minimum: 2,
      terms: ["\\braised\\b", "\\bfloating\\b", "\\bmodal\\b", "\\belevation\\b", "돌출", "올라온", "부유", "떠 있는", "모달", "고도", "그림자 단계"]
    }
  ];

  for (const check of tokenChecks) {
    const content = getSection(text, check.aliases) ?? "";
    if (countTerms(content, check.terms) < check.minimum) {
      fail(check.rule, file, `${check.aliases[0]}에는 raw 값이 아니라 semantic role이 필요합니다.`);
    }
  }

  const states = getSection(text, ["Component States", "컴포넌트 상태"]) ?? "";
  const requiredStates = [
    ["focus", /(?:\bfocus\b|초점|포커스)/i],
    ["disabled", /(?:\bdisabled\b|비활성)/i],
    ["loading", /(?:\bloading\b|로딩)/i],
    ["error", /(?:\berror\b|오류)/i]
  ];
  for (const [state, pattern] of requiredStates) {
    if (!pattern.test(states)) fail("missing-component-state", file, `Component States에 ${state} 적용성 기록이 없습니다.`);
  }
  if (/(?:not applicable|n\/?a|해당 없음)/i.test(states) && !/(?:reason|because|이유|대체)/i.test(states)) {
    fail("state-applicability-reason", file, "해당 없는 state에는 이유 또는 대체 feedback이 필요합니다.");
  }
}
...
function validateManifest(manifestRel) {
  ...
  checkDesignSystemEvidence(manifestRel, text);
  ...
  if (report !== undefined) {
    ...
    checkDesignSystemEvidence(primary, report);
  }
}
