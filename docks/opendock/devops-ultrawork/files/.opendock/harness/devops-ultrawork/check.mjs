...
const runSlug = "devops-ultrawork";
...
const config = {
  "title": "DevOps Ultrawork",
  "focus": "DevOps and infrastructure quality gates",
  "patterns": [
    {
    ...
    },
    {
      ...
      "pattern": "0\\.0\\.0\\.0/0",
      "message": "Public CIDR requires explicit review."
    },
    {
    ...
    },
    {
      ...
      "message": "Terraform auto-approve needs review."
    },
    {
      "id": "secret-in-infra",
      "globs": [
      ...
      "pattern": "(?i)(secret|password|token)\\s*[=:]\\s*\"[^$][^\"]{6,}\"",
      "message": "Infrastructure files must not contain literal secrets."
    },
    {
      "id": "missing-runbook-copy",
      "globs": [
        "**/*.md"
      ],
...
const ignoredRootFiles = new Set(["AGENTS.md", "CLAUDE.md", "GEMINI.md", ".opendock/docks/devops-ultrawork/HARNESS.md", ".opendock/docks/devops-ultrawork/README.md"]);
...
function deploymentMissingResources(text) {
  return text.split(/^---\s*$/m).some((document) => {
    if (!/^\s*kind:\s*Deployment\s*$/m.test(document)) return false;
    return !/^\s*resources:\s*$/m.test(document);
  });
}
...
function run() {
  ...
  for (const file of files) {
    if (!matchesAny(file.rel, ["**/*.yml", "**/*.yaml"])) continue;
    if (deploymentMissingResources(file.text)) {
      failures.push({
        rule: "missing-k8s-resource-limits",
        file: file.rel,
        detail: "Kubernetes deployments should define resource requests/limits."
      });
    }
  }
