...
const slug = "travel-journal";
const title = "Travel Journal";
const outputRoot = "travel-journal";
...
const requiredSections = [
  "기록 범위와 기간 (Journal Scope and Period)",
  "원본 사진·메모 목록 (Source Photo and Note Inventory)",
  "시간선·장소 근거 (Timeline and Place Evidence)",
  "인물·동의 (People and Consent)",
  "하이라이트·캡션·회고 계획 (Highlights, Captions, Reflection Plan)",
  "사실 불확실성 (Factual Uncertainty)",
  "개인정보 최소화·가림 (Data Minimization and Redaction)",
  "최종 이야기 변형 (Final Story Variants)",
  targetHeading,
  "검증 결과 (Validation)",
];
const outputHeadings = [
  ["시간선", /시간선|timeline/i],
  ["장소", /장소|places?/i],
  ["인물과 동의", /인물.*동의|동의.*인물|people.*consent|consent.*people/i],
  ["하이라이트", /하이라이트|highlights?/i],
  ["캡션", /캡션|captions?/i],
  ["사실 불확실성", /사실 불확실성|불확실한 사실|factual uncertainty/i],
  ["회고", /회고|reflection/i],
  ["개인정보와 가림", /개인정보.*가림|가림.*개인정보|privacy.*redaction|redaction.*privacy/i],
  ["원본 사진·메모 목록", /원본 사진.*메모 목록|사진·메모 목록|source photo.*note inventory|photo.*note inventory/i],
  ["최종 이야기 변형", /최종 이야기 변형|이야기 버전|final story variants?|story versions?/i],
];
const failures = [];
...
function validateOutput(targets, run) {
  ...
  const inventory = section(combined, "원본 사진·메모 목록") || section(combined, "Source Photo and Note Inventory");
  const inventoryItems = inventory.match(/^\s*[-*]\s+(?:photo|note)-[A-Za-z0-9._-]+/gim) ?? [];
  if (inventoryItems.length < 2 || !/photo-/i.test(inventory) || !/note-/i.test(inventory)) {
    fail("source-inventory", targets[0].relative, "사진과 메모를 포함한 안전한 ID의 원본 inventory가 두 항목 이상 필요합니다.");
  }
  const timelineItems = cleaned.match(/^\s*[-*]\s+\d{4}-\d{2}-\d{2}(?:\s+\d{1,2}:\d{2})?/gm) ?? [];
  if (timelineItems.length < 2) fail("timeline-evidence", targets[0].relative, "날짜가 있는 시간선 항목이 두 개 이상 필요합니다.");
  if (!/(?:동의|consent)\s*:\s*(?:예|아니오|미확인|yes|no|unknown)/i.test(cleaned) || !/(?:가명|익명|alias|pseudonym)/i.test(cleaned)) {
    fail("people-consent", targets[0].relative, "인물별 가명과 공개 동의 상태가 필요합니다.");
  }
  if (!/(?:확인됨|확인된 사실|confirmed|verified)/i.test(cleaned) || !/(?:기억상|추정|from memory|estimated)/i.test(cleaned) || !/(?:확인 필요|불확실|needs verification|uncertain)/i.test(cleaned)) {
    fail("factual-uncertainty", targets[0].relative, "확인된 사실, 기억·추정과 확인 필요 항목을 구분해야 합니다.");
  }
  const variants = section(combined, "최종 이야기 변형") || section(combined, "Final Story Variants");
  if (!/(?:짧은 버전|공유용 버전|short version|share version)/i.test(variants) || !/(?:긴 버전|개인 보관용 버전|long version|private archive version)/i.test(variants)) {
    fail("story-variants", targets[0].relative, "짧은 공유용과 긴 개인 보관용 이야기 변형이 모두 필요합니다.");
  }
  if (!/(?:여행 전|이전)[^.\n]{0,80}(?:여행 후|이후)|(?:생각|관점)[^.\n]{0,50}(?:달라|변화)|before the trip[^.\n]{0,100}after the trip|(?:view|perspective|thinking)[^.\n]{0,60}(?:changed|shifted)/i.test(cleaned)) {
    fail("reflection", targets[0].relative, "여행 전후 생각이나 관점의 변화를 담은 회고가 필요합니다.");
  }
  const consentBody = section(run.text, "인물·동의 (People and Consent)");
  const locationConsent = /위치 메타데이터 공개 동의\s*:\s*(?:예|yes)/i.test(consentBody);
  if (/(?:GPS|위도|경도|latitude|longitude)\s*[:=]\s*-?\d{1,3}\.\d{3,}/i.test(cleaned) && !locationConsent) {
    fail("location-metadata", targets[0].relative, "정밀 위치 메타데이터는 명시적 공개 동의 없이 포함할 수 없습니다.");
  }
  const identityConsent = /실명 공개 동의\s*:\s*(?:예|yes)/i.test(consentBody);
  if (/(?:실명|full name)\s*[:=]\s*[가-힣]{2,4}(?:\s|$)/im.test(cleaned) && !identityConsent) {
    fail("identity-metadata", targets[0].relative, "실명은 명시적 공개 동의 없이 포함할 수 없습니다.");
  }
  const deviceConsent = /기기 메타데이터 공개 동의\s*:\s*(?:예|yes)/i.test(consentBody);
  if (/(?:EXIF\s*)?(?:camera owner|artist|device serial|camera serial|카메라 소유자|기기 일련번호)\s*[:=]\s*\S{4,}/i.test(cleaned) && !deviceConsent) {
    fail("identity-metadata", targets[0].relative, "식별 가능한 기기 메타데이터는 명시적 동의 없이 포함할 수 없습니다.");
  }
  if (/(?:기억나지 않|불확실|확인하지 못)[^.\n]{0,40}(?:사실로 확정|확실한 사실)|fabricat(?:e|ed) as fact/i.test(cleaned)) {
    fail("fabricated-fact", targets[0].relative, "불확실한 기억을 확인된 사실로 만들 수 없습니다.");
  }
}
...
function validateRun(run) {
  ...
  const inventoryEvidence = section(run.text, "원본 사진·메모 목록 (Source Photo and Note Inventory)");
  if (!/photo-[A-Za-z0-9._-]+/i.test(inventoryEvidence) || !/note-[A-Za-z0-9._-]+/i.test(inventoryEvidence)) {
    fail("run-inventory", file, "run 근거에 사진과 메모의 안전한 ID inventory가 필요합니다.");
  }
  const consentEvidence = section(run.text, "인물·동의 (People and Consent)");
  if (!/(?:가명|익명|alias|pseudonym)/i.test(consentEvidence) || !/(?:동의|consent)/i.test(consentEvidence)) {
    fail("run-consent", file, "run 근거에 가명과 인물별 공개 동의 상태가 필요합니다.");
  }
