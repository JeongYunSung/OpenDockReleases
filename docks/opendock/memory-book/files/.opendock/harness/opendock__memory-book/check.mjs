...
const domainSections = [
  ["기간과 테마", "Period and Theme"],
  ["출처 목록", "Source Inventory"],
  ["타임라인", "Timeline"],
  ["인물 동의와 개인정보", "People Consent and Privacy"],
  ["장소와 안전한 세분성", "Places and Safe Granularity"],
  ["하이라이트", "Highlights"],
  ["캡션", "Captions"],
  ["회고", "Reflection"],
  ["가림과 제외", "Redactions and Exclusions"],
  ["최종 서사와 출력 계획", "Final Narrative and Output Plan"],
  ["검증과 불확실성", "Verification and Uncertainty"],
];
...
function explicitRun(argument) {
  ...
  const pattern = /^\.opendock\/runs\/memory-book\/([A-Za-z0-9._-]+)\/manifest\.md$/;
...
function actionableText(text) {
  ...
  let previous = "";
  const safeContext = /(실행하지\s*(?:않|말)|따르지\s*(?:않|말)|(?:명령|문구).{0,12}(?:금지|차단)|(?:금지|차단)(?:됨|한다|했다)|만들지\s*않|창작하지\s*않|보장하지\s*않|do not (?:run|execute|follow|invent|fabricate)|don't (?:run|execute|follow|invent|fabricate)|never (?:run|execute|follow|invent|fabricate)|must not (?:run|execute|follow|invent|fabricate)|prohibited command|blocked instruction|not fabricated)/i;
  return text.split(/\r?\n/).map((line) => {
    if (fence === null) {
      const opening = parseFenceOpening(line);
      if (opening) {
...
function validateCommonText(text, file) {
  ...
  const secretPatterns = [
    ...
    /(?:account(?:\s+(?:number|no\.?))?|card\s+number|계좌번호|카드번호|주민등록번호|주민번호|여권번호|운전면허번호)\s*[:=]\s*(?!\[?(?:redacted|masked|none|없음|미저장|가림|omitted|not stored)\]?)[A-Z0-9][A-Z0-9 -]{5,}/i,
  ];
  if (secretPatterns.some((pattern) => pattern.test(text))) addFailure("secret-value", file, "Remove credential, account, or full personal identifier values.");
  ...
  const fabricated = /(?:자료|근거|source).{0,24}(?:없지만|없이).{0,24}(?:기억|사건|발언|감정).{0,24}(?:만들|창작|꾸며)|(?:invented|fabricated|made up)\s+(?:memory|event|quote|emotion)/is;
  if (fabricated.test(actionable)) addFailure("fabricated-memory", file, "Do not invent memories, events, quotes, or emotions without source evidence.");

  const guarantee = /(?:100%|완벽히|절대).{0,15}(?:정확한 기억|기억 복원|진실)|guaranteed\s+(?:accurate|complete)\s+memory/i;
  if (guarantee.test(actionable)) addFailure("unsupported-guarantee", file, "Do not guarantee complete or perfectly accurate memory reconstruction.");
}

function sourceIds(body) {
  return new Set([...body.matchAll(/\[S(\d+)\]/gi)].map((match) => `S${match[1]}`.toUpperCase()));
}

function validateReferencedList(body, inventoryIds, file, label) {
  const lines = body.split(/\r?\n/).filter((line) => /^\s*[-*]\s+\S/.test(line));
  if (lines.length === 0) {
    addFailure("source-reference", file, `${label} needs at least one source-linked list item.`);
    return;
  }
  for (const line of lines) {
    const refs = sourceIds(line);
    if (refs.size === 0 || [...refs].some((id) => !inventoryIds.has(id))) {
      addFailure("source-reference", file, `${label} item must cite an ID declared in Source Inventory.`);
      return;
    }
  }
}

function validateMemoryBook(text, file) {
  const period = section(text, ["기간과 테마", "Period and Theme"]);
  const dates = period.match(/\b20\d{2}(?:-\d{2}-\d{2})?\b/g) ?? [];
  if (dates.length < 2 || !/(테마|주제|theme)/i.test(period) || !/(독자|공유|audience|sharing)/i.test(period)) {
    addFailure("period-theme", file, "Period needs two boundaries, a theme, audience, and sharing scope.");
  }

  const inventory = section(text, ["출처 목록", "Source Inventory"]);
  const inventoryIds = sourceIds(inventory);
  const inventoryRequirements = [/(사진|메모|편지|인터뷰|photo|note|letter|interview)/i, /\b20\d{2}(?:-\d{2}-\d{2})?\b/, /(제공자|소유자|provider|owner)/i, /(동의|consent)/i];
  if (inventoryIds.size === 0 || !inventoryRequirements.every((pattern) => pattern.test(inventory))) {
    addFailure("source-inventory", file, "Source inventory needs IDs, type, date, provider/owner, and consent status.");
  }
  if (/https:\/\//i.test(inventory) && (!/(조회일|확인일|accessed|access date)/i.test(inventory) || !/\b20\d{2}-\d{2}-\d{2}\b/.test(inventory))) {
    addFailure("source-url-date", file, "Web sources need an explicit access date.");
  }

  const timeline = section(text, ["타임라인", "Timeline"]);
  if (!/\b20\d{2}(?:-\d{2}-\d{2})?\b/.test(timeline)) addFailure("timeline-date", file, "Timeline entries need dates or years.");
  validateReferencedList(timeline, inventoryIds, file, "Timeline");
  validateReferencedList(section(text, ["하이라이트", "Highlights"]), inventoryIds, file, "Highlights");
  validateReferencedList(section(text, ["캡션", "Captions"]), inventoryIds, file, "Captions");

  const people = section(text, ["인물 동의와 개인정보", "People Consent and Privacy"]);
  const peopleTerms = [/(동의|consent)/i, /(공유 범위|sharing scope)/i, /(확인|제한|비공개|가명|confirmed|restricted|private|pseudonym)/i];
  if (!peopleTerms.every((pattern) => pattern.test(people))) {
    addFailure("people-consent", file, "People section needs consent status, sharing scope, and privacy treatment.");
  }

  const places = section(text, ["장소와 안전한 세분성", "Places and Safe Granularity"]);
  if (!/(도시|지역|국가|city|region|country)/i.test(places) || !/(정확한 주소|숙소|GPS|좌표|exact address|lodging|coordinates)/i.test(places) || !/(제외|가림|삭제|excluded|redacted|removed)/i.test(places)) {
    addFailure("place-granularity", file, "Places need city/region/country granularity and explicit exact-location exclusion.");
  }

  const redactions = section(text, ["가림과 제외", "Redactions and Exclusions"]);
  const redactionTerms = [/(인물|이름|person|name)/i, /(위치|주소|GPS|location|address)/i, /(EXIF|metadata)/i, /(가림|제외|삭제|redact|exclude|remove)/i];
  if (!redactionTerms.every((pattern) => pattern.test(redactions))) {
    addFailure("redaction-evidence", file, "Redactions need people, location, EXIF/metadata, and action evidence.");
  }

  const plan = section(text, ["최종 서사와 출력 계획", "Final Narrative and Output Plan"]);
  const planTerms = [/(형식|format)/i, /(독자|audience)/i, /(순서|구성|order|structure)/i, /(공유|sharing)/i, /(동의|consent)/i];
  if (!planTerms.every((pattern) => pattern.test(plan))) addFailure("output-plan", file, "Final plan needs format, audience, order, sharing scope, and consent check.");

  const verification = section(text, ["검증과 불확실성", "Verification and Uncertainty"]);
  if (!/(확인된|verified|confirmed)/i.test(verification) || !/(미확인|충돌|추정|uncertain|conflict|unverified)/i.test(verification) || !/(제외|표시|확인 담당자|exclude|label|reviewer)/i.test(verification)) {
    addFailure("verification-uncertainty", file, "Separate verified facts from unverified/conflicting material and state treatment.");
  }

  const sensitiveLocation = /(?:GPS\s*(?:Latitude|Longitude)|위도|경도)\s*[:=]\s*-?\d{1,3}\.\d{4,}|\b-?\d{1,3}\.\d{5,}\s*,\s*-?\d{1,3}\.\d{5,}\b|(?:집 주소|자택 주소|촬영 주소|home address|exact address|숙소|hotel|항공편|flight number|여행 일정|travel itinerary)\s*[:=]\s*(?!\[?(?:가림|redacted|omitted|미저장)\]?)[^\r\n]{5,}|\/Users\/[^\s]+|[A-Za-z]:\\Users\\[^\s]+/i;
  if (sensitiveLocation.test(text)) addFailure("sensitive-location", file, "Remove EXIF/GPS coordinates, exact addresses, lodging, itineraries, and personal absolute paths.");
}
...
function validateRun(run) {
  ...
  validateMemoryBook(run.text, run.relative);
  const targetPaths = extractTargets(run.text);
  if (targetPaths.length === 0) addFailure("target-list-empty", run.relative, "Target Files must list at least one memories/ Markdown file.");
  ...
  for (const target of targets) {
    ...
    validateMemoryBook(target.text, target.relative);
  }
...
function main() {
  ...
  if (runs.length === 0) {
    ...
    console.log("Ready: no active memory-book run found.");
    return;
  }
  ...
  console.log("Memory book quality gate passed.");
}
